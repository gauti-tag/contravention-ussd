module SessionManager
  ...
  def self.fetch_parameters
    #parameters = eval(REDIS_CLIENT.get('ussdsessions:contraventionparameters').to_s)
    #return parameters if parameters.present?

    api = Mmgg::FetchRecords.call(params: {model_code: 'Parameter'})
    parameters = api['data']

    #REDIS_CLIENT.setex('ussdsessions:contraventionparameters', 10.seconds.to_i, parameters.to_s)
    parameters
  rescue => e 
    Rails.logger.debug(e.inspect)
    []
